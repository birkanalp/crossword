...
// Used by both the public endpoint and admin routes.
// ---------------------------------------------------------------------------
...
export async function fetchLeaderboardEntries(
  ...
  if (type === "all_time") {
    // Best score (or fastest time) per user across all levels.
    // We fetch all entries and aggregate in memory.
    // For production scale, replace this with a DB view or RPC.
  ...
  if (type === "daily") {
    // Look up which level is assigned to this date in daily_challenges
