-- =============================================================================
-- Migration: 008_freq_score_generator_filter
-- Description: Add freq_score IS NOT NULL filter to eligible CTE in
--              run_generation_job_once() so only frequency-scored words
--              are selected for puzzle generation.
-- Date: 2026-02-25
-- =============================================================================
...
BEGIN
  ...
  eligible AS (
    ...
    WHERE w.language = v_job.language
      ...
      AND w.freq_score IS NOT NULL  -- only pick frequency-scored words
  ),
  picked AS (
    SELECT e.id
